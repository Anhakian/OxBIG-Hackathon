...
def get_health_guidelines():
    ...
    submit_query_body_exercise = {
        "endpointId": "predefined-openai-gpt4o",
        "query": "What are the recommended official guidelines for minutes of exercise and step number for adults? Please provide the output in 1 line, such as 'at least 150 minutes of moderate-intensity aerobic activity throughout the week, and 10,000 steps per day', each separated by a comma. Additionally, please provide the numerical values for each item as a combined list, after the initial response",
        "pluginIds": ["plugin-1712327325", "plugin-1713962163"],
        "responseMode": "sync"
    }
    ...
    nut_vals = eval(guidelines['nutrition']['data']['answer'].split('.')[1].strip().replace('\n', '') if '.' in guidelines['nutrition']['data']['answer'] else '')
    sleep_vals = eval(guidelines['sleep']['data']['answer'].split('.')[1].strip().replace('\n', '') if '.' in guidelines['sleep']['data']['answer'] else '')
    exercise_vals = eval(guidelines['exercise']['data']['answer'].split('.')[1].strip().replace('\n', '') if '.' in guidelines['exercise']['data']['answer'] else '')

 
    return guidelines, nut_vals, sleep_vals, exercise_vals
...
class WellnessApp:
    def __init__(self):
        ...
        self.health_guidelines, self.nut_vals, self.sleep_vals, self.exercise_vals = get_health_guidelines()
...
'''
        ...
        exercise_rec = self.health_guidelines['exercise']
        if user['exercise'] < int(self.exercise_vals[0]) and user['exercise_steps'] < int(self.exercise_vals[1]):
            story += f'''Realizing they had only moved for {user['exercise']} minutes and walked {user['exercise_steps']} steps today, Alex recalled the recommendations of {exercise_rec['data']['answer'][0]}. '''
        elif user['exercise'] < int(self.exercise_vals[0]):
            story += f'''Realizing they had only moved for {user['exercise']} minutes today, Alex recalled the recommendation of {exercise_rec['data']['answer'][0]}. They decided to start small by adding a 10-minute walk during their lunch break.'''
        elif user['exercise_steps'] < int(self.exercise_vals[1]):
            story += f'''Realizing they had only walked {user['exercise_steps']} steps today, Alex recalled the recommendation of {exercise_rec['data']['answer'][0]}. '''

        if user['exercise'] < int(self.exercise_vals[0]) or user['exercise_steps'] < int(self.exercise_vals[1]):
...
'''
    ...
    def get_nutrition_recommendation(self, user_id, food_input):
        return self.nutrition_agent.knowyourfood(food_input, self.health_guidelines['nutrition'], self.nut_vals)
...
class NutritionAgent:
    def knowyourfood(self, food_input, guidelines, nut_vals): 
        ...
        submit_query_body = {
            ...
            {guidelines}
            What should I do to improve my diet? Can you provide me with some meal suggestions? Please write the answer in third person past tense about someone called Alex, eg. Alex tried to incorporate more protein, so ate a bowl of greek yoghurt with 30g of protein or something similar.''',
            "pluginIds": ["plugin-1712327325", "plugin-1713962163"],
            "responseMode": "sync"
        }
